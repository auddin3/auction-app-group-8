					...
					<div class="card mt-4" style="width: 60rem">
						...
						<div class="card-title">
							<p class="card-text price-colour"
								><strong>Start Price:</strong> £{{ item.start_price }}</p
							>
						</div>

						<div class="card-title">
							<p class="card-text"
								><strong>End of Bid:</strong> {{ item.end_of_bid.substring(0, 10) }} &nbsp;
								{{ item.end_of_bid.substring(11, 19) }}</p
							>
						</div>
						...
						<button @click="deleteItem(item.id)" type="button" class="btn btn-outline-danger"
							><b>Delete</b></button
						>
						<br />
					</div>
...
export default {
	data() {
		...
		return {
			...
			owner: "",
			edit: false,
			new_product_image: "/product-images/stock-image.png",
		};
	},
	computed: {
		filteredItems() {
	...
	methods: {
		...
		async deleteItem(id: number) {
			let response = await fetch("http://localhost:8000/auctionapp/api/product/" + id, {
				method: "DELETE",
				body: JSON.stringify({
					id: id,
				}),
